...
"""
    ...
    def fall(self, pos: Vector2, new_hero_pos: Vector2, dx: float) -> None:
        while self.get(pos) in self.pushable_tiles and pos.y >= 0:
            self.plummet(pos, new_hero_pos, dx)
            pos -= Vector2(0, 1)
            dx = 0
    ...
    def plummet(self, pos: Vector2, new_hero_pos: Vector2, push_dx: float) -> None:
        new_pos = None
        dx = push_dx
        block = self.get(pos)
    ...
    def try_move(self, delta: Vector2) -> bool:
        ...
        if block == Tile.FOOD:
            ...
            new_pushedpos = newpos + delta
            if delta.y == 0 and self.get(new_pushedpos) == Tile.EMPTY:
                new_block_pos = newpos + delta
                self.set(newpos, Tile.EMPTY)
                self.set(new_block_pos, block)
                if self.get(new_block_pos + Vector2(0, 1)) in self.fall_into_tiles:
                    self.fall(new_block_pos, newpos, delta.x)
        elif block in self.move_onto_tiles:
            return True
        return False
        ...
        for y in range(self.level_height - 1, 0, -1):
            for x in range(self.level_width):
                ...
                if block == Tile.EMPTY:
                    ...
                    if block_above in self.pushable_tiles:
                        self.fall(pos_above, self.hero.position, 0)
